    ...
    mifosX.controllers = _.extend(module, {
        EditLoanAccAppController: function (scope, routeParams, resourceFactory, location, dateFilter, uiConfigService) {
            ...
            resourceFactory.loanResource.get({loanId: routeParams.id, template: true, associations: 'charges,collateral,meeting,multiDisburseDetails',staffInSelectedOfficeOnly:true}, function (data) {
                ...
                // The loan-by-id template above doesn't return the product's CLI rate slabs,
                // so fetch them from the product template (same call loanProductChange uses) and merge them in.
                var cliSlabParams = { resourceType: 'template', productId: data.loanProductId, templateType: scope.templateType, staffInSelectedOfficeOnly: true };
                if (scope.clientId) {
                    cliSlabParams.clientId = scope.clientId;
                }
                if (scope.groupId) {
                    cliSlabParams.groupId = scope.groupId;
                }
                resourceFactory.loanResource.get(cliSlabParams, function (productTemplateData) {
                    scope.loanaccountinfo.cliChargeSlabs = productTemplateData.cliChargeSlabs || [];
